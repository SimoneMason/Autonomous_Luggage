...
# Set multiplier value for motor speed
multiplier = 1
...
while True:
    ...
    try:
        while True:
            ...
            if (Gx or Gy or Gz) < 3 and (Gx or Gy or Gz) > -2:
                current_time = time.time()
                elapsed_time = current_time-start_time
                if elapsed_time - old_elapsed_time >= 0.5:
                        velocity_x = old_velocity + (Ax * 9.81 * (elapsed_time - old_elapsed_time))
                        max_velocity = max(max_velocity, velocity_x)
                        print(f"Max Velocity: {max_velocity:.2f} m/s")
                        old_velocity=velocity_x
                        old_elapsed_time = current_time-start_time
                ...
                # path is clear, follow person
                    ...
                    ret, frame = cap.read()
                    if not ret:
                        prinT("Could not get ret")
                        break
                    
                    highlighted_frame, center = highlight_yellow_color(frame)
                    
                    if center is None:
                        print("Cannot find desired colour in frame")
                        p1.start(0)
                        p2.start(0)
                    else:
                        
                        (mid_point_X, y) = center
                        print("center: ", center)
                        ...
                        if angle >= 0 :
                            ...
                            # ensures that motors do not error from being provided vlaue out of rnage (0,100)
                            if angle > 100:
                                angle = 90
                            GPIO.output(DIG1, GPIO.HIGH)		
                            GPIO.output(DIG2, GPIO.LOW)	
